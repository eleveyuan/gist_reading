...
real Model::binaryLogistic(int32_t target, bool label, real lr) {  // 二元逻辑斯蒂损失
...
real Model::negativeSampling(int32_t target, real lr) {  // 负采样损失
  real loss = 0.0;
  grad_.zero();
...
real Model::hierarchicalSoftmax(int32_t target, real lr) {  // 层次softmax损失
  real loss = 0.0;
  grad_.zero();
...
void Model::setTargetCounts(const std::vector<int64_t>& counts) {
  ...
  if (args_->loss == loss_name::ns) {  // nagetive sampling
    initTableNegatives(counts);
  }
  if (args_->loss == loss_name::hs) {  // hierarchical softmax 
    buildTree(counts);
  }
}

void Model::initTableNegatives(const std::vector<int64_t>& counts) {  // 初始化负采样表
  real z = 0.0;
...
int32_t Model::getNegative(int32_t target) {  // 获取负样本
  int32_t negative;
  do {
...
void Model::buildTree(const std::vector<int64_t>& counts) {  // 构建霍夫曼树的方式与word2vec一样，只是存储方式不一样
  ...
  int32_t leaf = osz_ - 1;
  int32_t node = osz_;
  for (int32_t i = osz_; i < 2 * osz_ - 1; i++) {  // 构建树
    int32_t mini[2];
...
real Model::getLoss() const {  // 计算损失
  return loss_ / nexamples_;
}

void Model::initSigmoid() {  // 初始化t_sigmoid表
...
real Model::log(real x) const {  // 查log表
  if (x > 1.0) {
    return 0.0;
  }
...
real Model::sigmoid(real x) const {  // 查sigmoid表
  if (x < -MAX_SIGMOID) {
    return 0.0;
