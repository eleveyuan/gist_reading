...
void Args::parseArgs(const std::vector<std::string>& args) {  // 解析参数
...
void Args::printDictionaryHelp() {  // 词典参数
  std::cerr
...
void Args::printTrainingHelp() {  // 训练基础参数
  std::cerr
...
void Args::printQuantizationHelp() {  // quantize
  std::cerr
...
void Args::save(std::ostream& out) {  // 保存命令行参数
...
void Args::load(std::istream& in) {  // 加载命令行参数
