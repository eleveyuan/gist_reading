...
int32_t Dictionary::find(const std::string& w) const {  // 中间层，查找词是否存在，返回单词的id
...
void Dictionary::add(const std::string& w) {  // 添加单词
  int32_t h = find(w);
  ntokens_++;
  if (word2int_[h] == -1) {
    entry e;
    e.word = w;
    e.count = 1;
    e.type = getType(w);  // 获取单词的类型: 标签还是词
    words_.push_back(e);
    word2int_[h] = size_++;  //  hash-index表，存储单词在单词表中的索引
  } else {
    words_[word2int_[h]].count++;  // 出现过的单词，统计词频
  }
}

int32_t Dictionary::nwords() const {  // 词典中单词数
  return nwords_;
}

int32_t Dictionary::nlabels() const {  // 标签数
  return nlabels_;
}

int64_t Dictionary::ntokens() const {  // 语料中单词数
  return ntokens_;
}

const std::vector<int32_t>& Dictionary::getSubwords(int32_t i) const {  // 根据索引获取单词的子串
  assert(i >= 0);
  assert(i < nwords_);
  return words_[i].subwords;
}

const std::vector<int32_t> Dictionary::getSubwords(  // 根据单词，进行切割获取子串
...
int32_t Dictionary::getId(const std::string& w, uint32_t h) const {  // 获取(单词, hash)在单词表的id
  int32_t id = find(w, h);
  return word2int_[id];
}

int32_t Dictionary::getId(const std::string& w) const {  // 获取单词id
  int32_t h = find(w);
  return word2int_[h];
}

entry_type Dictionary::getType(int32_t id) const {  // 根据id获取单词表中单词的类型
  assert(id >= 0);
  assert(id < size_);
  return words_[id].type;
}

entry_type Dictionary::getType(const std::string& w) const {  // 根据word的子串获取word的类型
  return (w.find(args_->label) == 0) ? entry_type::label : entry_type::word;  // stl函数，查找word的子串是否为"__label__",是则返回子串开始位置0
}

std::string Dictionary::getWord(int32_t id) const {  // 根据id获取单词表中单词
  assert(id >= 0);
  assert(id < size_);
  return words_[id].word;
}

uint32_t Dictionary::hash(const std::string& str) const {  //  计算单词hash值
...
void Dictionary::computeSubwords(const std::string& word,
  ...
  for (size_t i = 0; i < word.size(); i++) {
    std::string ngram;
    if ((word[i] & 0xC0) == 0x80) continue;  // 处理utf-8多字节字符 https://stackoverflow.com/questions/3911536/utf-8-unicode-whats-with-0xc0-and-0x80
    for (size_t j = i, n = 1; j < word.size() && n <= args_->maxn; n++) {
      ngram.push_back(word[j++]);
      while (j < word.size() && (word[j] & 0xC0) == 0x80) {  // 处理多字节序的字符
        ngram.push_back(word[j++]);
      }
      if (n >= args_->minn && !(n == 1 && (i == 0 || j == word.size()))) {
        int32_t h = hash(ngram) % args_->bucket;  // 分桶
        pushHash(ngrams, h);
      }
    }
  }
...
void Dictionary::initNgrams() {  // 初始化字符级n-grams
...
bool Dictionary::readWord(std::istream& in, std::string& word) const  // 读取单词
{
  char c;
...
void Dictionary::threshold(int64_t t, int64_t tl) {  // 去掉一些出现低频的词
...
std::vector<int64_t> Dictionary::getCounts(entry_type type) const {  // 获取type的counts
...
void Dictionary::pushHash(std::vector<int32_t>& hashes, int32_t id) const {
  ...
  if (pruneidx_size_ > 0) {
    if (pruneidx_.count(id)) {  // 是否存在id这个key
      id = pruneidx_.at(id);  // 获取key值为id的value值
    } else {
      return;
    }
  }
