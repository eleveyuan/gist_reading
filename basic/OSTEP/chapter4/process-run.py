...
class scheduler:
    def __init__(self, process_switch_behavior, io_done_behavior, io_length):
        ...
        self.proc_info = {}
        self.process_switch_behavior = process_switch_behavior  # 进程转换行为
        self.io_done_behavior = io_done_behavior  # io完成行为
    ...
    def load_program(self, program):  # 加载-p参数 单个进程
        ...
        for line in program.split(','):
            ...
            if opcode == 'c':  # compute
                num = int(line[1:])
                for i in range(num):
                    self.proc_info[proc_id][PROC_CODE].append(DO_COMPUTE)  # 进行cpu
            elif opcode == 'i':
                self.proc_info[proc_id][PROC_CODE].append(DO_IO)  # 进行io
            else:
                print('bad opcode %s (should be c or i)' % opcode)
                exit(1)
        return

    def load(self, program_description):  # 加载-l参数 多个进程
        proc_id = self.new_process()
    ...
    def get_num_active(self):  # 获取还有多少进程没有完成
        num_active = 0
    ...
    def check_if_done(self):  # 检查进程是否完成，然后转交给下一个进程执行
    ...
    def run(self):
        ...
        while self.get_num_active() > 0:
            ...
            # check for io finish  # 检查io任务是否完成
            io_done = False
            for pid in range(len(self.proc_info)):
            ...
            if self.proc_info[self.curr_proc][PROC_STATE] == STATE_RUNNING and \
                ...
                instruction_to_execute = self.proc_info[self.curr_proc][PROC_CODE].pop(0)  # 把当前进程的一条指令弹出
                cpu_busy += 1
            ...
            if instruction_to_execute == DO_IO:  # 切换至阻塞态，等待io任务的完成
...
if __name__ == '__main__':
    ...
    # PARSE ARGUMENTS
    #
    parser = OptionParser()  # 解析命令行参数
    """
    -s 随机数种子
    -P 
    -l 逗号分割的进程列表 x1:y1,x2:y2,... x为指令数，y指令使用 CPU 或发出 IO 的机会
    -L
    -S
    -I 
    -c 进行实际的调度运行，获取计算结果
